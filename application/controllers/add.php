...
if (!isset($_POST['submit'])) {
    ...
    //////Populate group perm list///////////////// (no existing file, so no rights yet — just the picklist)
    ...
    $GLOBALS['smarty']->assign('can_set_high_classification', $user_obj->canSetHighClassification($current_user_dept));
    ...
    for ($count = 0; $count<$numberOfFiles; $count++) {
        ...
        // Validate classification — only Admin/Super-Admin/Dept Head may assign Sensitive or Highly sensitive
        $requested_classification = isset($_REQUEST['classification']) ? $_REQUEST['classification'] : 'Internal';
        $restricted_levels = array('Sensitive', 'Highly sensitive');
        if (in_array($requested_classification, $restricted_levels) && !$user_obj->canSetHighClassification($current_user_dept)) {
            header('Location: error?ec=30&last_message=' . urlencode('Only Admins or Department Heads may classify a document as Sensitive or Highly sensitive'));
            exit;
        }

        // Staged Approval floor check (Hybrid workflow model, 2026-08-11 decision):
        // Sensitive/Highly sensitive documents must have a workflow template with
        // enough stages assigned at upload time — this is what gives the ICMH
        // classification scheme real teeth rather than being metadata-only.
        // sanitizeme() (odm-init.php) turns a blank submitted field into boolean
        // false, not '' — a strict '!== ""' check misses that, so use !empty()
        // to correctly treat both '' and false as "no template selected".
        $requested_workflow_template_id = (!empty($_REQUEST['workflow_template_id'])) ? (int) $_REQUEST['workflow_template_id'] : null;
        $required_stages = WorkflowTemplate::getRequiredStageCount($requested_classification);
        if ($required_stages > 0) {
            if (!$requested_workflow_template_id) {
                header('Location: error?ec=36&last_message=' . urlencode($requested_classification . ' documents require a workflow template with at least ' . $required_stages . ' stage(s) to be assigned.'));
                exit;
            }
            ...
            if ($requested_template->getStageCount() < $required_stages) {
                header('Location: error?ec=36&last_message=' . urlencode('The selected workflow template only has ' . $requested_template->getStageCount() . ' stage(s), but ' . $requested_classification . ' requires at least ' . $required_stages . '.'));
                exit;
            }
        }
        ...
        // Auto-generated, traceable serial number (DEPT-YEAR-NNNN), scoped
        // per department per calendar year.
        $generated_serial_number = FileData::generateSerialNumber($current_user_dept, $pdo);
        ...
        $file_data_stmt->bindParam(':serial_number', $generated_serial_number);
        ...
        $file_data_stmt->bindParam(':classification', $requested_classification);
        ...
        if ($requested_workflow_template_id) {
            $new_file_obj = new FileData($fileId, $pdo);
            $new_file_obj->setWorkflowTemplate($requested_workflow_template_id, $pdo);
        }
        ...
        // Insert Group Rights into group_perms (absent if no groups exist yet)
        if (isset($_REQUEST['group_permission'])) {
            foreach ($_REQUEST['group_permission'] as $group_id => $group_permission) {
                $group_perms_query = "INSERT INTO {$GLOBALS['CONFIG']['db_prefix']}group_perms (fid, group_id, rights) VALUES($fileId, :group_id, :permission)";

                $group_perms_stmt = $pdo->prepare($group_perms_query);
                $group_perms_stmt->bindParam(':group_id', $group_id);
                $group_perms_stmt->bindParam(':permission', $group_permission);
                $group_perms_stmt->execute();
            }
        }
