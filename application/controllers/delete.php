...
if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'tmpdel') {
    // This action mutates state (archives the file) but is reached via a
    // same-origin GET link/redirect (details.php's delete link, and
    // rejects.php's server-side redirect after its own POST already
    // validated). A GET request can't carry a form-POSTed CSRF token, so
    // the caller embeds the token pair as query params instead — bridge
    // them into $_POST here, since AntiCSRF's validateRequest() only ever
    // reads $_POST directly, regardless of what's passed to validateToken().
    if (isset($_GET['_CSRF_INDEX'], $_GET['_CSRF_TOKEN'])) {
        $_POST['_CSRF_INDEX'] = $_GET['_CSRF_INDEX'];
        $_POST['_CSRF_TOKEN'] = $_GET['_CSRF_TOKEN'];
    }
    if (isset($GLOBALS['csrf']) && !$GLOBALS['csrf']->validateToken(null, '/delete')) {
    ...
    if (isset($GLOBALS['csrf']) && !$GLOBALS['csrf']->validateToken($_POST, '/delete')) {
        header('Location: error?ec=1&last_message=' . urlencode('CSRF token validation failed'));
        exit;
    }
...
} elseif (isset($_POST['submit']) && $_POST['submit'] == 'Undelete') {
    if (isset($GLOBALS['csrf']) && !$GLOBALS['csrf']->validateToken($_POST, '/delete')) {
        header('Location: error?ec=1&last_message=' . urlencode('CSRF token validation failed'));
        exit;
    }
    ...
    if (isset($_REQUEST['checkbox'])) {
        foreach ($_REQUEST['checkbox'] as $fileId) {
            ...
            AccessLog::addLogEntry($fileId, 'U', $pdo);
        }
    }
...
function pmt_delete($id)
{
    ...
    if (isset($id)) {
        ...
        if ($userperm_obj->canAdmin($id)) {
            // Log before deletion, while the file's data row (and Ver/Rev) still exist
            AccessLog::addLogEntry($id, 'P', $pdo);
