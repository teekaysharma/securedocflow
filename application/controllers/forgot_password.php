...
if (
    ...
    // Same redirect message whether or not the username exists — a
    // different message here ("username not found" vs "email sent") would
    // let anyone enumerate valid usernames just by submitting this form.
    if ($stmt->rowCount() == 0) {
        ...
        $randstring = '';
        while ($i <= 7) {
            // random_int(), not rand() — this code is the sole secret
            // proving reset authorization, so it needs to be
            // unpredictable, not just "looks random".
            $num = random_int(0, 62);
