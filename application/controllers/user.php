...
if (isset($_REQUEST['submit']) and $_REQUEST['submit'] == 'adduser') {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if (isset($_POST['department_head'])) {
            for ($i = 0; $i < sizeof($_POST['department_head']); $i++) {
                $dept_head = $_POST['department_head'][$i];
                $query = "INSERT INTO {$GLOBALS['CONFIG']['db_prefix']}dept_head (dept_id, user_id) values(:dept_head, :user_id)";
                $stmt = $pdo->prepare($query);
                $stmt->execute(array(
                    ':dept_head' => $dept_head,
                    ':user_id' => $user_id
    ...
    if (@$GLOBALS['CONFIG']['demo'] == 'True') {
        ...
        $query = "SELECT dept_id, user_id FROM {$GLOBALS['CONFIG']['db_prefix']}dept_head where user_id = :user_id";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(':user_id' => $_REQUEST['item']));
        $dept_head = $stmt->fetchAll();
        ...
        // Same pattern as the reviewer-department options above, for the
        // Department Head multi-select (odm_dept_head instead of dept_reviewer).
        $department_head_select_options = array();
        for ($d = 0; $d < sizeof($all_departments); $d++) {
            $found = false;
            foreach ($dept_head as $row) {
                if ($all_departments[$d][0] == $row['dept_id']) {
                    $department_head_select_options[] = '<option value="' . e::h($all_departments[$d][0]) . '" selected>' . e::h($all_departments[$d][1]) . '</option>';
                    $found = true;
                    break;
                }
            }
            if (!$found) {
                $department_head_select_options[] = '<option value="' . e::h($all_departments[$d][0]) . '">' . e::h($all_departments[$d][1]) . '</option>';
            }
        }
        ...
        $GLOBALS['smarty']->assign('department_head_select_options', $department_head_select_options);
    ...
    if ($user_obj->isAdmin()) {
        ...
        $query = "DELETE FROM {$GLOBALS['CONFIG']['db_prefix']}dept_head WHERE user_id = :user_id";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(':user_id' => $_POST['id']));

        if (isset($_REQUEST['department_head'])) {
            for ($i = 0; $i < sizeof($_REQUEST['department_head']); $i++) {
                $query = "INSERT INTO {$GLOBALS['CONFIG']['db_prefix']}dept_head (dept_id,user_id) VALUES(:dept_id, :user_id)";
                $stmt = $pdo->prepare($query);
                $stmt->execute(array(
                    ':dept_id' => $_REQUEST['department_head'][$i],
                    ':user_id' => $_POST['id']
                ));
            }
        }
