...
// Cancel Check-Out — must run before the "already checked out" guard below,
// since that guard would otherwise block this very action.
if (isset($_GET['submit']) && $_GET['submit'] == 'cancel') {
    $cancel_file_obj = new FileData($_REQUEST['id'], $pdo);
    $cancel_user_obj = new User($_SESSION['uid'], $pdo);
    $is_self_cancel = ($cancel_file_obj->getStatus() == $_SESSION['uid']);
    if ($is_self_cancel || $cancel_user_obj->isAdmin()) {
        $query = "UPDATE {$GLOBALS['CONFIG']['db_prefix']}data SET status = 0 WHERE id = :id";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(':id' => (int) $_REQUEST['id']));
        AccessLog::addLogEntry((int) $_REQUEST['id'], $is_self_cancel ? 'C' : 'B', $pdo);
        header('Location: details?id=' . (int) $_REQUEST['id'] . '&last_message=' . urlencode('Check-out cancelled'));
        exit;
    } else {
