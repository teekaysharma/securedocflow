...
<?php
    // Human-readable labels for every access-log action code
    $history_actions_array = array(
        "A" => 'Added',
        "B" => 'Check-Out Force-Cancelled (Admin)',
        "C" => 'Check-Out Cancelled',
        "V" => 'Viewed',
        "D" => 'Downloaded',
        ...
        "Y" => 'Approved',
        "R" => 'Rejected',
        "E" => 'Validity Date Set/Renewed',
        "U" => 'Restored from Archive',
        "L" => 'Classification Changed',
        "P" => 'Permanently Deleted',
        "S" => 'Workflow Stage Approved',
        "Q" => 'Access Requested',
        "G" => 'Access Request Granted',
        "N" => 'Access Request Denied'
    );

    $unified_events = array();

    // query to obtain a list of check-in/upload revisions (odm_log)
    ...
    foreach ($result as $row) {
        ...
        if (is_file($GLOBALS['CONFIG']['revisionDir'] . $id . '/' . $id . "_$revision.dat")) {
            $version_html = '<a href="details?id=' . e::h($id) . '_' . e::h($revision) . '&state=' . (e::h($_REQUEST['state'])) . '"><div class="revision">' . e::h(($revision + 1)) . '</div></a>' . e::h($extra_message);
        } else {
            $version_html = e::h($revision) . e::h($extra_message);
        }

        $unified_events[] = array(
            'timestamp' => $row['modified_on'],
            'version_html' => $version_html,
            'by' => e::h($row['last_name']) . ', ' . e::h($row['first_name']),
            'action' => 'Checked In / Uploaded',
            'note' => e::h($row['note'])
        );
    }

    // Merge in the access log (view, download, checkout, approve/reject, classification, validity,
    // archive/restore, etc.) — only for the default (non-revision-scoped) view, since matching these
    // events to "as of a specific past revision" is ambiguous and not attempted here.
    if (!isset($revision_id)) {
        $access_query = "
          SELECT
            a.action,
          ...
          INNER JOIN
            {$GLOBALS['CONFIG']['db_prefix']}user u ON a.user_id = u.id
          WHERE
            a.file_id = :id
          ORDER BY
            a.timestamp DESC
        ...
        $access_stmt->execute(array(':id' => $id));
        $access_result = $access_stmt->fetchAll();

        foreach ($access_result as $arow) {
            if ($arow['doc_version'] === null) {
                $version_html = '—';
            } else {
                $version_html = 'Ver-' . e::h($arow['doc_version']) . ' Rev-' . e::h($arow['doc_revision']);
            }

            $action_label = isset($history_actions_array[$arow['action']]) ? $history_actions_array[$arow['action']] : $arow['action'];
            $note = $arow['reason'] !== null ? e::h($arow['reason']) : '(No reason recorded)';

            $unified_events[] = array(
                'timestamp' => $arow['timestamp'],
                'version_html' => $version_html,
                'by' => e::h($arow['last_name']) . ', ' . e::h($arow['first_name']),
                'action' => $action_label,
                'note' => $note
            );
        }
    ...
    // Sort the merged timeline newest-first
    usort($unified_events, function ($a, $b) {
        return strtotime($b['timestamp']) <=> strtotime($a['timestamp']);
    });

    foreach ($unified_events as $event) {
        ...
        echo '<td align=center><font size="-1">' . $event['version_html'] . '</font></td>';
        echo '<td><font size="-1">' . fix_date($event['timestamp']) . '</font></td>';
        echo '<td><font size="-1">' . $event['by'] . '</font></td>';
        echo '<td><font size="-1">' . e::h($event['action']) . '</font></td>';
        echo '<td><font size="-1">' . $event['note'] . '</font></td>';
        echo '</tr>';
    }
    // clean up
?>
