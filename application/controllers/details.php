...
$file_detail_array = array(
    ...
    // Wrap with a plain newline, not raw '<br />' — the template HTML-escapes
    // this value (correctly, since it's user-supplied text), which was turning
    // the literal '<br />' into visible '&lt;br /&gt;' for any description or
    // comment over 50 characters. The template applies |nl2br after |escape
    // to turn these newlines into real line breaks post-escaping.
    'description' => wordwrap($description, 50, "\n"),
    'comment' => wordwrap($comment, 50, "\n"),
    'udf_details_display' => udf_details_display($request_id),
    'revision' => $revision,
    'ver_rev' => $file_data_obj->getVersionLabel(),
    'file_under_review' => $file_under_review,
    'reviewer' => $reviewer,
    'status' => $status,
    'serial_number' => $file_data_obj->getSerialNumber()
);

// Staged Approval progress, shown on Details when a workflow template is assigned
if ($file_data_obj->getWorkflowTemplateId()) {
    $workflow_template_obj = new WorkflowTemplate($file_data_obj->getWorkflowTemplateId(), $pdo);
    $current_stage_num = $file_data_obj->getWorkflowStageNumber();
    $stage_count = $workflow_template_obj->getStageCount();
    $stages = $workflow_template_obj->getStages();
    $current_stage_name = '';
    foreach ($stages as $s) {
        if ((int) $s['stage_number'] === (int) $current_stage_num) {
            $current_stage_name = $s['name'];
            break;
        }
    }
    $GLOBALS['smarty']->assign('workflow_template_name', $workflow_template_obj->getName());
    $GLOBALS['smarty']->assign('workflow_stage_label', 'Stage ' . $current_stage_num . ' of ' . $stage_count . ($current_stage_name !== '' ? ' — ' . $current_stage_name : ''));
}
...
// Sensitive/Highly sensitive documents: while checked out, only the lock-holder may view content.
// Everyone else sees a checked-out notice with a contact link instead of the View action.
$is_checkout_locked_for_viewing = false;
$is_lock_holder = false;
$current_user_obj_early = new User($_SESSION['uid'], $pdo);
$high_class = array('Sensitive', 'Highly sensitive');
if ($status > 0 && in_array($file_data_obj->getClassification(), $high_class)) {
    $is_checkout_locked_for_viewing = true;
    $is_lock_holder = ($status == $_SESSION['uid']);
    $GLOBALS['smarty']->assign('is_checkout_locked_for_viewing', true);
    $GLOBALS['smarty']->assign('is_lock_holder', $is_lock_holder);
    $GLOBALS['smarty']->assign('cancel_checkout_link', 'check-out?id=' . $request_id . '&submit=cancel');
    $GLOBALS['smarty']->assign('check_in_link', 'check-in?id=' . $request_id);
    $GLOBALS['smarty']->assign('force_cancel_available', $current_user_obj_early->isAdmin() && !$is_lock_holder);
}
...
if ($user_permission_obj->getAuthority($request_id, $file_data_obj) >= $user_permission_obj->READ_RIGHT) {
    ...
    // Not enough access to view this document at all (and not looking at a
    // past-revision view, where requesting access doesn't make sense) —
    // offer the Access Request workflow instead.
    $GLOBALS['smarty']->assign('request_access_link', 'access_request?submit=request&id=' . $request_id);
    $GLOBALS['smarty']->assign('has_pending_access_request', AccessRequest::hasPendingRequest($request_id, $_SESSION['uid'], $pdo));
}
...
// delete.php's tmpdel action mutates state but is only reachable via this
// GET link, which can't carry a form-POSTed CSRF field — embed the token
// pair as query params instead; delete.php bridges them into $_POST before
// validating.
if (isset($GLOBALS['csrf'])) {
    foreach ($GLOBALS['csrf']->getTokenArray('/delete') as $fieldName => $fieldValue) {
        $my_delete_link .= '&' . rawurlencode($fieldName) . '=' . rawurlencode($fieldValue);
    }
}
...
// Authorize/Reject shortcut — only when this document is pending AND the current user is Admin or the assigned Reviewer
$current_user_obj = new User($_SESSION['uid'], $pdo);
$can_review = ($file_data_obj->isPublishable() == 0) && ($current_user_obj->isAdmin() || $current_user_obj->isReviewerForFile($request_id));
$GLOBALS['smarty']->assign('can_review', $can_review);
if ($can_review) {
    $GLOBALS['smarty']->assign('review_file_id', $request_id);
    if (isset($GLOBALS['csrf'])) {
        $review_csrf_data = $GLOBALS['csrf']->getTokenForTemplate('/toBePublished');
        $GLOBALS['smarty']->assign('review_csrf_field', $review_csrf_data['field']);
    }
}
