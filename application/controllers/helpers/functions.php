...
function draw_header($pageTitle, $lastmessage = '')
{
    ...
    // Admin or Department Head of at least one department can review Access Requests
    if ($uid != null) {
        $GLOBALS['smarty']->assign('can_review_access_requests', $current_user_obj->isAdmin() || $current_user_obj->isDeptHeadOfAny($pdo));
    }
...
function list_files(array $fileid_array, object $userperms_obj, string $dataDir, $showCheckBox = false, $rejectpage = false)
{
    ...
    foreach ($fileid_array as $fileid) {
        ...
        $ver_rev = $file_obj->getVersionLabel();
        $serial_number = $file_obj->getSerialNumber();
