...
<?php
    ...
    if (isset($_POST["checkbox"])) {
        foreach ($_POST['checkbox'] as $num=>$cbox) {
            $fileid = $cbox;
            $url .= '&id' . $num . '=' . urlencode($fileid);
            $id ++;
            $loop++;
        }
    }
    // Bridge the CSRF token as query params onto this same-origin redirect —
    // delete.php's tmpdel action requires it (see its handler for why a GET
    // redirect can't carry a form-POSTed token instead).
    if (isset($GLOBALS['csrf'])) {
        foreach ($GLOBALS['csrf']->getTokenArray('/delete') as $fieldName => $fieldValue) {
            $url .= '&' . rawurlencode($fieldName) . '=' . rawurlencode($fieldValue);
        }
    }
    $url .= '&num_checkboxes=' . urlencode($loop);
    header('Location: ' . $url);
}
