...
class CsrfProtection
{
    ...
    private function __construct()
    {
        ...
        if (getenv('IS_DOCKER') && session_status() == PHP_SESSION_NONE) {
            ...
            // Only force the Secure flag off when the request genuinely
            // isn't HTTPS — see the matching comment in public/index.php.
    ...
    {
        if (empty($action)) {
            $action = $this->normalizeAction(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
        }
        try {
    ...
    /**
     * Raw index/token field-name => value pairs (e.g. ['_CSRF_INDEX' => '...',
     * '_CSRF_TOKEN' => '...']) for the rare case a caller needs to carry a
     * token somewhere other than a POST form's hidden fields — e.g. appended
     * as query-string params on a same-origin GET link/redirect that a
     * legacy action still requires, bridged into $_POST before validation
     * (validateToken()/AntiCSRF only ever read $_POST directly).
     *
     * @param string $action
     * @return array
     */
    public function getTokenArray($action = '')
    {
        if (empty($action)) {
            $action = $this->normalizeAction(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
        }
        try {
            return $this->csrf->getTokenArray($action);
        } catch (Exception $e) {
            error_log('CSRF Token Array Error: ' . $e->getMessage());
            return array();
        }
    }
...
<body>
    ...
    /**
     * Normalize the request path used as a CSRF token's action key.
     *
     * The app's front controller treats the site root and "/index" as the
     * same endpoint (forms use a relative action="index"), but the raw
     * REQUEST_URI path differs between the two, which previously caused
     * tokens generated on "/" to fail validation when posted to "/index".
     * Collapsing them to a single canonical key fixes that mismatch.
     *
     * @param string $path
     * @return string
     */
    private function normalizeAction($path)
    {
