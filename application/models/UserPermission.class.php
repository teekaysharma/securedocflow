...
if (!defined('UserPermission_class')) {
    ...
    class UserPermission extends databaseData
    {
        ...
        public function __construct($uid, PDO $connection)
        {
            ...
            // Group_Perms is keyed on user id (not group id) — a user can belong to
            // several groups, so it aggregates across all of them internally.
            $this->group_perms_obj = new Group_Perms($userId, $connection, $this->user_obj);
        ...
        public function getViewableFileIds($limit = true)
        {
            ...
            $group_perms_file_array = ($this->group_perms_obj->getCurrentViewOnly($limit));
            ...
            $group_perms_file_array = array_diff($group_perms_file_array, $array);
            $group_perms_file_array = array_diff($group_perms_file_array, $user_perms_file_array);
            $total_listing = array_merge($user_perms_file_array, $group_perms_file_array, $dept_perms_file_array);
        ...
        public function getReadableFileIds($limit = true)
        {
            ...
            $group_perms_file_array = $this->group_perms_obj->getCurrentReadRight($limit);
            $published_file_array = $this->user_obj->getPublishedData(1);
            $result_array = array_values(array_unique(array_merge($published_file_array, $user_perms_file_array, $group_perms_file_array, $dept_perms_file_array)));
            return $result_array;
        }
        ...
        public function getWritableFileIds($limit = true)
        {
            ...
            $group_perms_file_array = $this->group_perms_obj->getCurrentWriteRight($limit);
            $published_file_array = $this->user_obj->getPublishedData(1);
            $result_array = array_values(array_unique(array_merge($published_file_array, $user_perms_file_array, $group_perms_file_array, $dept_perms_file_array)));
            return $result_array;
        }
        ...
        public function getAdminableFileIds($limit = true)
        {
            ...
            $group_perms_file_array = $this->group_perms_obj->getCurrentAdminRight($limit);
            $published_file_array = $this->user_obj->getPublishedData(1);
            $result_array = array_values(array_unique(array_merge($published_file_array, $user_perms_file_array, $group_perms_file_array, $dept_perms_file_array)));
            return $result_array;
        }
        ...
        public function getAuthority($data_id)
        {
            ...
            $group_permissions = $this->group_perms_obj->getPermission($data_id);
            $department_permissions = $this->dept_perms_obj->getPermission($data_id);

            // Priority: individual override > group membership > department default.
            // Individual is the most specifically-targeted grant, department is the
            // broadest fallback — group sits between the two, same reasoning as why
            // individual already overrides department.
            if ($user_permissions >= $this->user_perms_obj->NONE_RIGHT and $user_permissions <= $this->user_perms_obj->ADMIN_RIGHT) {
                return $user_permissions;
            } elseif ($group_permissions >= $this->group_perms_obj->NONE_RIGHT and $group_permissions <= $this->group_perms_obj->ADMIN_RIGHT) {
