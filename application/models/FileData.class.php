...
if (!defined('FileData_class')) {
    ...
    */
    ...
    class FileData extends databaseData
    {
        ...
        public function loadData()
        {
            ...
            if ($stmt->rowCount() == $this->result_limit) {
                foreach ($result as $row) {
                    ...
                    $this->doc_version = (int) $row['doc_version'];
                    $this->doc_revision = (int) $row['doc_revision'];
                    $this->doc_classification = $row['doc_classification'];
                    $this->valid_until = $row['valid_until'];
                    $this->workflow_template_id = $row['workflow_template_id'];
                    $this->workflow_stage_number = $row['workflow_stage_number'];
        ...
        /**
         * return "Ver-X Rev-Y" label
         * @return string
         */
        public function getVersionLabel()
        {
            return 'Ver-' . $this->doc_version . ' Rev-' . $this->doc_revision;
        }
        ...
        /**
         * Change classification on an already-uploaded document. Admin/Super-Admin
         * only — stricter than the upload-time rule, since this is a governance
         * change to an existing, possibly already-approved document, not an initial
         * assignment. Logged with the reason via 'L' in the access log.
         */
        public function setClassification($new_classification, $reason, $pdo)
        {
            $query = "UPDATE {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_DATA SET doc_classification = :classification WHERE id = :id";
            $stmt = $this->connection->prepare($query);
            $stmt->execute(array(':classification' => $new_classification, ':id' => $this->id));
            $this->doc_classification = $new_classification;
            $this->setReviewerComments($reason);
            AccessLog::addLogEntry($this->id, 'L', $pdo, $reason);
        }
        ...
        /**
         * Is this document past its validity date? Null = never expires.
         */
        public function isExpired()
        {
            ...
            return strtotime($this->valid_until) < strtotime(date('Y-m-d'));
        }

        /**
         * Set/renew validity date. Logged via AccessLog with the reason, same
         * mechanism as the archive/restore movement-history logging.
         */
        public function setValidUntil($date, $reason, $pdo)
        {
            $query = "UPDATE {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_DATA SET valid_until = :valid_until WHERE id = :id";
            $stmt = $this->connection->prepare($query);
            $stmt->execute(array(':valid_until' => $date, ':id' => $this->id));
            $this->valid_until = $date;
            $this->setReviewerComments($reason);
            AccessLog::addLogEntry($this->id, 'E', $pdo, $reason); // 'E' = validity/expiration change
        }
         ...
         * @return int|null
         */
        public function getWorkflowStageNumber()
        {
            return $this->workflow_stage_number;
        }

        /**
         * Assign (or clear, with $templateId = null) a workflow template to this
         * document, resetting progress to stage 1. Floor-checking against the
         * document's classification (ICMH Sensitive/Highly sensitive require a
         * minimum stage count) is the caller's responsibility — see
         * WorkflowTemplate::getRequiredStageCount() — this method just persists
         * the assignment once that check has already passed.
         * @param int|null $templateId
         * @param PDO $pdo
         */
        public function setWorkflowTemplate($templateId, $pdo)
        {
            $stageNumber = $templateId ? 1 : null;
            $query = "UPDATE {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_DATA SET workflow_template_id = :template_id, workflow_stage_number = :stage_number WHERE id = :id";
            $stmt = $this->connection->prepare($query);
            $stmt->execute(array(':template_id' => $templateId, ':stage_number' => $stageNumber, ':id' => $this->id));
            $this->workflow_template_id = $templateId;
            $this->workflow_stage_number = $stageNumber;
        }

        /**
         * Advance to the next stage of this document's assigned workflow template.
         * Does not touch publishable — the caller (toBePublished.php) decides
         * whether this was the final stage and finalizes approval separately.
         * @param PDO $pdo
         */
        public function advanceWorkflowStage($pdo)
        {
            $this->workflow_stage_number = (int) $this->workflow_stage_number + 1;
            $query = "UPDATE {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_DATA SET workflow_stage_number = :stage_number WHERE id = :id";
            $stmt = $this->connection->prepare($query);
            $stmt->execute(array(':stage_number' => $this->workflow_stage_number, ':id' => $this->id));
        }
        ...
        /**
         * Generate the next traceable serial number for a document being
         * uploaded to $departmentId, in the form DEPT-YEAR-NNNN (e.g.
         * OPS-2026-0001), scoped per department per calendar year. The
         * department code is derived deterministically from the first word
         * of the department's name (first 3 letters, uppercased) rather than
         * stored anywhere, so this needs no admin configuration and works
         * automatically for any department, including ones created later.
         * @param int $departmentId
         * @param PDO $pdo
         * @return string
         */
        public static function generateSerialNumber($departmentId, $pdo)
        {
            $deptQuery = $pdo->prepare("SELECT name FROM {$GLOBALS['CONFIG']['db_prefix']}department WHERE id = :id");
            $deptQuery->execute(array(':id' => $departmentId));
            $deptName = $deptQuery->fetchColumn();

            $firstWord = preg_replace('/[^A-Za-z]/', '', explode(' ', trim((string) $deptName))[0]);
            $deptCode = strtoupper(substr($firstWord !== '' ? $firstWord : 'GEN', 0, 3));

            $year = date('Y');
            $prefix = $deptCode . '-' . $year . '-';

            $seqQuery = $pdo->prepare("
              SELECT serial_number FROM {$GLOBALS['CONFIG']['db_prefix']}data
              WHERE serial_number LIKE :prefix
              ORDER BY serial_number DESC
              LIMIT 1
            ");
            $seqQuery->execute(array(':prefix' => $prefix . '%'));
            $lastSerial = $seqQuery->fetchColumn();

            $nextSeq = 1;
            if ($lastSerial) {
                $lastSeqPart = (int) substr($lastSerial, strlen($prefix));
        ...
        /**
         * Increment revision on check-in. Revision 0-9, then rolls to
         * next version and resets revision to 0.
         */
        public function incrementRevision()
        {
            if ($this->doc_revision >= 9) {
                $this->doc_version += 1;
                $this->doc_revision = 0;
            } else {
                $this->doc_revision += 1;
            ...
            $stmt->execute(array(
                ...
                ':doc_revision' => $this->doc_revision,
                ':id' => $this->id
            ));
        }
