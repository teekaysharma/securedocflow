...
class InstallerController
{
    ...
    public function handleRequest(): void
    {
        ...
        // Once the application has already been installed (its schema
        // exists), every installer operation — including a fresh reinstall
        // that drops all tables, and schema upgrades — must be restricted to
        // an authenticated Admin. Without this gate, any unauthenticated
        // visitor could hit ?op=install&force_fresh=1 and destroy the entire
        // live database. A brand-new deployment (no schema yet) has no admin
        // to check against, so this only applies post-install.
        if ($this->dbManager->odmsysTableExists($prefix) && !$this->isAuthenticatedAdmin()) {
            http_response_code(403);
            $this->renderError('This application is already installed. Installer access requires an authenticated Admin session.');
            return;
        }
    ...
    /**
     * Is the current request from a logged-in Admin? Used to gate every
     * installer operation once the app is already installed. session_start()
     * already ran at the top of this file; the User class autoloads via the
     * global spl_autoload_register registered in public/index.php.
     * @return bool
     */
    private function isAuthenticatedAdmin(): bool
    {
        ...
        try {
            ...
            $user = new User((int) $_SESSION['uid'], $pdo);
            return $user->isAdmin();
        } catch (Exception $e) {
            return false;
        }
