<form name="update" id="modifyUserForm" action="user" method="POST" enctype="multipart/form-data">
    ...
    <table border="0" cellspacing="5" cellpadding="5">
        ...
        <tr id="userDeptHeadRow">
            <td id="userDeptHeadLabelTd">Department Head For</td>
            <td id="userDeptHeadListTd">
                <select class="multiView" id="userDeptHeadList" name="department_head[]" multiple="multiple" {$mode} >
                    {foreach from=$department_head_select_options item=item name=department_head_select_options}
                        {$item}
                    {/foreach}
                </select>
                &nbsp;(can classify Sensitive/Highly sensitive documents and set validity dates for this department)
            </td>
        </tr>
        <tr>
